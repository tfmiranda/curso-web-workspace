...
exports.visualizarUsuario = function(req, res) {
  const { id } = req.params;
  Usuario.findById(id, function(err, usuario) {
    if (!err) {
      res.render('usuarios/perfil', { usuario });
    } else {
      req.flash('warning', 'Nenhum usuário encontrado');
      res.redirect('back');
    }
  });
};
...
exports.editarUsuario = function(req, res) {
  ...
  if (!id) {
    ...
    Usuario.findOneAndUpdate({ _id: usuario._id }, usuario, opcoes, function(err, novoUsuario) {
      if (err) {
