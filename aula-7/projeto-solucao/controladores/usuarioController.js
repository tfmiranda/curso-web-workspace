...
exports.cadastrarUsuario = function(req, res) {
    ...
    Usuario.register(novoUsuario, req.body.senha, function(err) {
        if (err) {
            req.flash('warning', 'Erro ao registrar usuário: ' + err.message);
            res.redirect('back');
        }
        else {
...
// Middleware que verifica se o usuario solicitante e o mesmo usuario
// que deseja acessar
exports.isMesmoUsuario = function(req, res, next) {
    const idRequested = req.params.id;
    const idAtual = req.user.id;
    if (idRequested === idAtual) {
        next();        
    } else {
        req.flash('danger', 'Não é permitido editar outro usuário');
        res.redirect('back');   
    }
